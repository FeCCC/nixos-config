...
in
{
  ...
  proxy-groups = [
    ...
    (mkUrlTest "♻️ 自动选择" filterSpecial)
    (mkFallback "🔯 故障转移" filterSpecial)
    (
      {
        name = "🔮 负载均衡";
        type = "load-balance";
    ...
    (mkSelect "📲 电报消息" allProxies)
    (mkSelect "💬 Ai平台" allExceptSpecial)
    (mkSelect "📹 油管视频" allProxies)
    (mkSelect "🎥 奈飞视频" ([ "🎥 奈飞节点" ] ++ allExceptSpecial))
    (mkSelect "📺 巴哈姆特" [
      "台湾节点"
      "🚀 节点选择"
      "🚀 手动切换"
    ...
    (mkSelect "🌍 国外媒体" allProxies)
    (mkSelect "🌏 国内媒体" ([ "DIRECT" ] ++ allProxies))
    (mkSelect "📢 谷歌FCM" ([ "DIRECT" ] ++ allProxies))
    (mkSelect "Ⓜ️ 微软Bing" ([ "DIRECT" ] ++ allExceptSpecial))
    (mkSelect "Ⓜ️ 微软云盘" ([ "DIRECT" ] ++ allExceptSpecial))
    (mkSelect "Ⓜ️ 微软服务" ([ "DIRECT" ] ++ allExceptSpecial))
    (mkSelect "🍎 苹果服务" ([ "DIRECT" ] ++ allExceptSpecial))
    (mkSelect "🎮 游戏平台" ([ "DIRECT" ] ++ allExceptSpecial))
    (mkSelect "🎶 网易音乐" ([ "DIRECT" ] ++ allExceptSpecial))
    (mkSelect "🎯 全球直连" [
      "DIRECT"
      "🚀 节点选择"
    ])
    ...
    (mkUrlTest "香港节点" filterHK)
    (mkUrlTest "日本节点" filterJP)
    (mkUrlTest "美国节点" filterUS)
    (mkUrlTest "台湾节点" filterTW)
    (mkUrlTest "狮城节点" filterSG)
    (mkUrlTest "韩国节点" filterKR)
    ({
      name = "🎥 奈飞节点";
      type = "select";
      include-all = true;
